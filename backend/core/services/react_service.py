...
class ReActLoopService:
    ...
    async def run(
        ...
        while loop_count < config.max_loops:
            ...
            while retry_count <= REACT_MAX_RETRIES:
                try:
                ...
                except (ProviderError, TransientError) as e:
                    retry_count += 1
                    if retry_count <= REACT_MAX_RETRIES:
                    ...
                    _log.error("LLM retries exhausted", error=str(e)[:200], retries=retry_count)
                    if full_response.strip():
                        break
                    fallback = self._get_fallback_response(str(e).lower())
                    full_response = fallback
                    yield ChatEvent(EventType.TEXT, fallback)
                    break

                except AxnmihnError as e:
                    # Non-retryable structured error
                    _log.error("LLM permanent error", error=str(e)[:200])
                    if full_response.strip():
                        break
                    fallback = self._get_fallback_response(str(e).lower())
                    full_response = fallback
